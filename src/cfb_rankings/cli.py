...
def build_parser() -> argparse.ArgumentParser:
    ...
    gen_thread_parser.add_argument(
        ...
        help="Skip interactive prompts. Currently writes a draft scaffold; "
             "live LLM call wiring lands in a future sprint.",
    )
...
def main() -> None:
    ...
    if args.command == "generate-thread-chapter":
        ...
        drafts_dir = Path("src/cfb_rankings/storylines/seeds/_drafts")
        drafts_dir.mkdir(parents=True, exist_ok=True)
        draft_path = drafts_dir / f"{slug}_{next_n:02d}_{ts}.py"
        scaffold = (
            f'"""Draft chapter scaffold for {slug} chapter {next_n}.\n\n'
            f'Generated by `manage.py generate-thread-chapter`. '
            f'Live LLM-call wiring is queued for a follow-on sprint; this '
            f'scaffold gives a human reviewer the structure to fill in.\n'
            f'"""\n'
            f'THREAD_SLUG = "{slug}"\n\n'
            f'CHAPTERS = [\n'
            f'    {{\n'
            ...
            f'        "dek": "...",\n'
            f'        "body_markdown": """...""",\n'
            f'        "byline": "From The {meta["title"]} Department",\n'
            f'        "published_at": "{datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")}",\n'
        ...
        print(f"draft scaffold written: {draft_path}")
        print(f"  thread: {meta['title']}")
        print(f"  next chapter number: {next_n}")
        print("  fill in the scaffold, then move into the live seeds module + re-run render-storylines.")
        return
