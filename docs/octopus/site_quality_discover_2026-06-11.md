...
> **Divergence is broader than the two 404'd hubs (added 2026-06-11 after independent repo-grounded review).** The box `build_publish.ps1` omits ~15 commands that the cloud `publish_site.yml` runs — including **`manage.py prediction-ledger --action resolve`** (`cli.py:7543`; `resolve_due_predictions`) at `publish_site.yml:402-404` and **`manage.py backfill-edition-citations`** at `publish_site.yml:441-443`, plus `scrape-wiki-awards`, `ingest-cfbd-coaches`, `ingest-nfl-draft`, `refresh-award-watch/-depth-chart/-portal-heat/-recruiting-pulse`, `render-canon-*`, `build-search-index`, `repair-team-current-identity`, `sync-team-locations`, and several `seed-*`. **This reframes two of the "empty table" findings:** `prediction_ledger`=0 and `editorial_citations`=0 are **the same root cause as the offseason/film-room 404** — the producing command exists and runs in the retiring cloud path but not in the canonical box deployer. (This is *distinct* from `predictive_claims` (266), which genuinely has **no** resolver wired anywhere — see §1 #9.) **Implication:** the Phase-0 manifest must reconcile the entire box-vs-cloud *command* set, not just the two visible routes, or Gate A/B will close the 404s while silently leaving `prediction_ledger`, `editorial_citations`, award-watch, depth-chart, etc. un-refreshed on every box deploy.
