...
| 0.2b | Reconcile safe box-omitted RENDER gaps (search-index; canon/daily/edition) | ✅ done | `d910da5`+ | **WIRED `build-search-index`** (site search was DEAD — index file absent, cmdk.js on every page). Fixed 2 indexer bugs found validating it: `index_conferences` (real schema + gate → 72 confs) and **`index_players` URL (was `/players/{id}.html` → 8,660 player results would 404; now `{slug}-{id}.html`)**. Real-data: 9,483 items, **0 dead links** (was ~91% dead). **NOT wired** render-canon-all (0 rows→clobber) / render-daily (empty today→clobber); render-edition DEFERRED (visual). 75 tests pass |
...
- **`index_players` URL bug fixed (critical — would have shipped ~91% dead search links).** The validation pass found `index_players` emitted `/players/{player_id}.html`, but real pages are `{slugify(full_name)}-{player_id}.html` (e.g. `/players/fernando-mendoza-12763.html`) — so **all 8,660 player results 404'd** (91% of the 9,483-item index). `reporting.py::_player_slug`'s own docstring even names "search index" as a link that must resolve, so this was a regression. Fixed `index_players` to build the URL via `slugify` (from `cfb_rankings.utils`, mirroring `_player_slug`) and gate on page existence when `site_dir` is known (player_id is unstable across re-ingest → linkrot; drop rather than ship a dead link; logs the drop count — no silent cap). **Verified against real data: 8,660 player items, 0 of them 404** (was 8,660/8,660 dead). Updated the test asserting the bare-id fiction; added a player existence-gate test.
...
**Files:** `scripts/build_publish.ps1` (+1 Run step, PS parses clean), `scripts/build_manifest.py` (COMMAND_PARITY decisions recorded; `build-search-index` → `box`), `src/cfb_rankings/cmdk/index_builder.py` (conferences + players URL fix + `site_dir` thread), `tests/test_cmdk_index_builder.py` (+3 tests, 2 corrected).
**Verification:** `pytest tests/test_cmdk_index_builder.py tests/test_cmdk_assets.py tests/test_prediction_markets.py` → **75 passed**; full suite collects 1828, no import breakage; real-data scratch-render = 9,483 items, **0 dead links across every kind** (72 conferences + 8,660 players all resolve, was 8,660 player 404s); PS parse OK; scratch files removed; **no DB mutation, no deploy.**
**Net effect:** site-wide search goes from *dead* (index file absent) to *fully working with 0 broken links*. Without the player fix, wiring would have shipped ~91% dead search results — caught and fixed in the same WP.
**Blast radius:** one new non-Critical build step + two indexer functions. **Rollback:** drop the `build-search-index` Run line; `git checkout` the index_builder/test changes.
