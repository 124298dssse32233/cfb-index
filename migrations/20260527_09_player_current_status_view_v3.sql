-- Wave 25 / Phase 1 fix — Player Current Status View v3
--
-- v2 added 2025-roster inference for Type A. v3 adds name-alias resolution
-- for draft data: when a player has a status_override pointing to NFL but
-- their own player_id has no draft row, look up draft data from another
-- player_id with the same name (resolved via the auto-alias script).
--
-- Concrete fix: Gabriel pid=11737 has 90 stat rows on Oregon and an override
-- to NFL_DRAFTED_PRIOR (from script wave25_seed_alias_overrides.py), but the
-- draft row lives on pid=120 (the canonical Gabriel record). v3 finds that
-- draft row via name match and surfaces it on pid=11737's view row.
...
-- Name-aliased NFL: best draft row for each NAME (any player_id with that name).
-- Used as fallback when the player's own pid has no draft row but an override
-- says they're in the NFL.
nfl_by_name AS (
    SELECT p.full_name,
           MAX(pnd.draft_year) AS latest_draft_year,
           MAX(CASE WHEN pnd.draft_year = 2026 THEN 1 ELSE 0 END) AS drafted_2026,
           (SELECT round FROM player_nfl_draft pnd2
              JOIN players p2 ON p2.player_id = pnd2.player_id
              WHERE p2.full_name = p.full_name
              ORDER BY pnd2.draft_year DESC LIMIT 1) AS round,
           (SELECT pick FROM player_nfl_draft pnd2
              JOIN players p2 ON p2.player_id = pnd2.player_id
              WHERE p2.full_name = p.full_name
              ORDER BY pnd2.draft_year DESC LIMIT 1) AS pick,
           (SELECT overall FROM player_nfl_draft pnd2
              JOIN players p2 ON p2.player_id = pnd2.player_id
              WHERE p2.full_name = p.full_name
              ORDER BY pnd2.draft_year DESC LIMIT 1) AS overall,
           (SELECT nfl_team FROM player_nfl_draft pnd2
              JOIN players p2 ON p2.player_id = pnd2.player_id
              WHERE p2.full_name = p.full_name
              ORDER BY pnd2.draft_year DESC LIMIT 1) AS nfl_team,
           (SELECT nfl_team_abbr FROM player_nfl_draft pnd2
              JOIN players p2 ON p2.player_id = pnd2.player_id
              WHERE p2.full_name = p.full_name
              ORDER BY pnd2.draft_year DESC LIMIT 1) AS nfl_team_abbr
...
SELECT
    ...
    -- NFL: prefer direct row on this player_id, fall back to name-alias when
    -- override points to NFL but direct draft is missing
    COALESCE(nfl_d.latest_draft_year, nfl_n.latest_draft_year) AS draft_year,
    COALESCE(nfl_d.round, nfl_n.round) AS draft_round,
    COALESCE(nfl_d.pick, nfl_n.pick) AS draft_pick,
    COALESCE(nfl_d.overall, nfl_n.overall) AS draft_overall,
    COALESCE(nfl_d.nfl_team, nfl_n.nfl_team) AS nfl_team,
    COALESCE(nfl_d.nfl_team_abbr, nfl_n.nfl_team_abbr) AS nfl_team_abbr,
