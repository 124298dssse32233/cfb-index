...
- **Finding (session 10):** `teams.state` is **empty in prod (0 rows)** — the footprint module's home-state highlighting (`_team_home_state`) silently never fires. Choropleth deliberately derives presence from recruit geography instead, so it's unaffected, but the `teams.state` ingest gap is worth closing for the chip highlight. Flagged, not fixed (data-ingest, out of this scope).
- **In flight:** None
