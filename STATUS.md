...
- **In flight:** None
- **Blocked:** Network (chart type #9) is the only remaining new type and is gated on the WS-06 `coaches` table (coaching-tree edges have no source until then). Transfer-origin choropleth surface is data-gated on `teams.state`, which is empty in prod until the next publish runs `sync-team-locations` — can't build/verify it against empty data yet.
- **Next action:** Build the Choropleth's second surface (transfer-origin geography) once `teams.state` populates on the next publish. Network type lands with WS-06.
