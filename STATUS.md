...
### WS-02 — Classification + state machinery (~40% complete)
...
- **In flight:** None
- **Blocked:** None
- **Next action:** (1) Wire `populate-arcs` into CI (`ingest_daily.yml`, after the classify-fanbases step) so arcs propagate to production — the populator is committed but not yet scheduled. (2) Build the 5 missing arc-frame feeds (coordinator OC/DC source, NIL event source, weekly CFP-projection feed, program_tier population) to light up the data-gated frames. (3) Consume `season_narrative_state` open-arcs in the Chronicle frame-stack selector / storyline candidate queue.
