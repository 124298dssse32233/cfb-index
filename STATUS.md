...
- **In flight:** None
- **Blocked:** None
- **Next action:** (1) Build the 5 missing arc-frame feeds (coordinator OC/DC source, NIL event source, weekly CFP-projection feed, program_tier population) to light up the data-gated frames. (2) Consume `season_narrative_state` open-arcs in the Chronicle frame-stack selector / storyline candidate queue.
