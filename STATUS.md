...
- **In flight:** None
- **Blocked:** Transfer-origin *choropleth* surface is still data-gated on `teams.state` (empty in prod until the next publish runs `sync-team-locations`) — genuinely can't build/verify against empty data. (Note: this is the choropleth's 2nd surface, distinct from the now-live portal *network*.)
- **Next action:** Build the Choropleth's second surface (transfer-origin geography) once `teams.state` populates on the next publish. WS-08 chart-vocabulary expansion (6→9) is otherwise complete.
