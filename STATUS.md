...
- **In flight:** Measuring the real gzip distribution across all ~19.7k pages to choose an enforce budget.
- **Blocked:** Not blocked (can audit current state anytime)
- **Next action:** Set `--enforce` budget from the measured distribution and wire `check_page_weight.py --enforce` into `publish_site.yml`. Then Phase 5 (March 2027). Until then: enforce mobile-rendering checks in PR review (baseline = clean per the session-9 audit above).
