...
- **In flight:** None
- **Blocked:** Transfer-origin *choropleth* surface is still data-gated on `teams.state` (empty in prod — verified 0/766 this session — until the next publish runs `sync-team-locations`); genuinely can't build/verify against empty data. (Note: this is the choropleth's 2nd surface, distinct from the now-live portal *network*.)
- **Next action:** (1) Migrate more chart surfaces through `render_chart_card` toward the running-gate "every chart renders through the shared component" + "annotation DSL ≥5 production examples." (2) Build the Choropleth's second surface (transfer-origin geography) once `teams.state` populates on the next publish. WS-08 chart-vocabulary expansion (6→9) is complete.
