...
- **In flight:** None
- **Blocked:** Network (chart type #9) is the only remaining new type and is gated on the WS-06 `coaches` table (coaching-tree edges have no source until then).
- **Next action:** Build the Choropleth's second surface (transfer-origin geography) and the annotation-overlay DSL when scheduled. Network type lands with WS-06.
