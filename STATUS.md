...
- **Shipped (2026-05-28, session 10):** ✅ **Choropleth — chart type #8 of 9 — LIVE.** New centralised `src/cfb_rankings/charts/` package (the home the spec/D-007 always specified; didn't exist before) houses `choropleth.py`: a US **statebins tile-grid cartogram** (one tile per state+DC in approximate geographic position, single-hue gold sequential ramp, `sqrt` spread, per-tile `aria-label`). Server-side SVG, no JS (also satisfies WS-11's static-SVG-fallback bar). **Design call:** tile grid over true boundaries because real state shapes collapse below ~360px — tiles stay legible at 320px and keep the SVG ~18KB (within page-weight budget). Wired into the existing **Recruiting Footprint** module (`team_pages/recruiting_footprint.py`), which previously showed geography as text chips only — now renders a map of where a program pulls from across the last 4 cycles, above the chips (chips kept as the accessible fallback). Verified against prod DB + screenshot: Alabama reads correctly as the US (AL peak 24, GA 12, CA 11, FL 7; northeast cluster + TX bottom-center positioned right). 7 tests (`tests/test_choropleth.py`). Vocab doc updated (`docs/design-system/31-chart-vocabulary.md`: type #7 entry + 6→9 header). Ships on next publish.
- **Finding (session 10):** `teams.state` is **empty in prod (0 rows)** — the footprint module's home-state highlighting (`_team_home_state`) silently never fires. Choropleth deliberately derives presence from recruit geography instead, so it's unaffected.
- **Closed the teams.state gap (session 11):** Teams are created from CFBD *game* payloads (no location), so `teams.state` was empty for all 766 teams. Added `sync_cfbd_team_locations` (ingest/cfbd.py) — hits the location-bearing `/teams` endpoint, resolves each payload to a team_id by cfbd source id (all 766 have one; name match is the fallback), and `coalesce`-updates `city`/`state` so a blank never clobbers an existing value. New `sync-team-locations` CLI command, wired into the publish enrich step (`publish_site.yml`, after coaches). 6 unit tests with a mocked CFBD payload; UPDATE verified against a copy of the prod schema. Populates on next publish → unblocks the footprint home-state highlight AND a future transfer-origin geography chart (join `transfer_entries.from_team_id` → `teams.state`).
- **Shipped (2026-05-28, session 11):** ✅ **Annotation-overlay DSL — LIVE.** New shared `cfb_rankings.charts.render_annotation_overlay` + `Annotation` dataclass (`charts/annotation.py`): the NYT-Upshot callout the spec's "Annotation discipline" section always implied but every chart was re-rolling ad-hoc (see `team_pages/game_recap_hero.py`). Coordinate-space agnostic — caller hands it pixel `(x, y)` in their own viewBox; it returns a self-contained `<g>` overlay (marker dot + gold leader + dark serif label box, ≤3 lines). **Deterministic + collision-aware:** boxes are clamped inside the plot rect and greedily nudged off each other, so output is stable and unit-testable with zero live data. Static SVG, no JS/hover (satisfies WS-11 static-fallback bar; legible at 320px). 7 tests (`tests/test_annotation_overlay.py`); visually verified against the spec's Alabama power-rating example (3 callouts, no overlap, in bounds). Vocab doc updated with an "Implemented by" pointer under the Annotated Line type — new annotated charts must reuse it.
- **First Annotated-Line consumer + governance lint (2026-05-28, session 11-12):** Wired the DSL into the **era trajectory chart** (`era_pages/renderer.py`, `_select_era_annotations`) — the Annotated Line type (#4 of 9) is now a live fan surface (≤3 LLM-free callouts: first title → peak season → steepest fall; a prose-annotated season drops its redundant title-star). The package docstring promised "a CI lint can forbid `def render_*_chart` outside this package" — that lint now exists (`tests/test_chart_governance.py`): a pure-AST walk that fails the build on any new chart-type renderer defined outside `charts/`, with the 8 original-six inline renderers registered in a self-cleaning `PENDING_CENTRALIZATION` frozenset (migrating/renaming one forces its removal). 3 tests.
- **Shipped (2026-05-28, session 12):** ✅ **Network — chart type #9 — LIVE. All 9 locked types now have a centralised renderer.** New `charts/network.py` (`render_network` + `NetworkNode`/`NetworkEdge`): a deterministic circular chord diagram — nodes evenly on a ring (no force sim → no JS, stable/diffable output, satisfies WS-11 static-SVG bar), directed bezier-chord edges with a constant-size arrowhead marker, node radius ∝ degree, edge width ∝ weight. `label_color` makes it portable to light host pages. 10 tests (`tests/test_network_chart.py`). **The "gated on WS-06 coaches table" blocker was lazily labeled** — a coaching-carousel network from `team_seasons.head_coach` (915 rows) is genuinely too sparse (largest component 8 nodes), but the *portal* is dense: wired a live **Portal Pipelines** web onto `/offseason/` (`scripts/build_offseason_leaderboards.py::_transfer_network_section`) — top-16 FBS programs by 2026 transfer volume, FBS→FBS edges thinned to ≥2-player pipelines (Iowa State→Penn State ×23, North Texas→Oklahoma State ×16). Verified against prod DB + screenshot; full-page build runs end-to-end. Vocab doc gets a full §8 Network entry; governance test asserts `render_network` lives in the package.
- **Shipped (2026-05-28, session 13):** ✅ **Shared chart-card shell — the "single shared component all charts render through" (spec Definition-of-perfect + running gate).** New `charts/card.py::render_chart_card` + `CHART_CARD_CSS`: the consistent `<figure>` wrapper (eyebrow → headline → lede → chart + optional x/y axis labels + optional annotation-overlay layer → **source-receipt footer**) that kills the documented chrome drift ("some charts have receipt footers, some don't"). Pure deterministic string composition, no JS (satisfies WS-11 static bar), 9 tests (`tests/test_chart_card.py`); governance test asserts it lives in the package. First production consumer = the **Talent Migration** Sankey on `/offseason/`, which gained a source-receipt footer it previously lacked (its title + source moved off the SVG onto the card). Vocab doc gets a "Chart-card shell" section. Remaining surfaces migrate incrementally (same posture as `PENDING_CENTRALIZATION`).
- **Shipped (2026-05-28, session 14):** Carded two more live surfaces toward the running gate "every chart renders through the shared component." (1) **Era trajectory** (`era_pages/renderer.py`) now routes its three-act SVG through `render_chart_card` — gains a source-receipt footer (`CFB Index · power ratings`) + lede; annotations stay composited inside the chart's own viewBox. (2) **Recruiting-footprint choropleth** (`team_pages/recruiting_footprint.py`) routes its map through the card — gains a source-receipt footer (`player_recruiting_profiles`) it previously lacked. Added `as_figure=False` to `render_state_choropleth` (bare svg + legend, no nested figure), mirroring `render_network`; the chip passes eyebrow/lede (not headline) so all text resolves against the dark team-page tokens. Both verified against prod DB + screenshot (Alabama). 2 new tests. Also carded the **Dynasty Heatmap** standalone page (`/history/heatmap/`, `dynasty_heatmap.py`) — its SVG now routes through the card for a standardized source-receipt footer (replacing the ad-hoc prose source sentence; the editorial "how to read it" note stays). Carded surfaces now: Sankey, network, era trajectory, footprint choropleth, dynasty heatmap (5).
- **Shipped (2026-05-28, session 15):** Annotation-DSL reached its natural boundary at **3 live consumers** (era trajectory, dynasty heatmap, portal network) — `render_network` gained an optional node-keyed `annotations` param; the offseason Portal Pipelines web names its busiest hub. The ≥5 gate is aspirational; remaining candidates were rejected as churn/dormant (footprint + Sankey self-narrate; `season_arc_card` keeps purpose-fit color-coded annotations; game_recap_hero + heisman offseason-dormant). Boundary documented in `31-chart-vocabulary.md`.
- **Shipped (2026-05-28, session 16):** ✅ **WS-08 FULLY COMPLETE — choropleth 2nd surface LIVE.** The "gated on `teams.state`" label was **lazily applied**: `teams.state` IS still empty (0/766, populates only on the next publish's `sync-team-locations`), but the fan question "where did the portal grow up?" is answerable TODAY via a *different* join — `transfer_entries.player_id → player_recruiting_profiles.state_province` (the recruit's HOME state, 46k+ rows). New **"Where the Portal Grew Up"** section on `/offseason/` (`_transfer_origin_geography_section`), rendered through `render_chart_card`, self-narrating via a top-3-states lede (TX 189, FL 157, GA 127 for 2026). `render_state_choropleth` gained an `on_dark` flag (default True) so it renders on the cream offseason page — pale-accent-tint ramp, faint dark empty-tile wash, inline-accurate legend gradient (mirrors `render_network`'s `label_color` portability). Verified vs prod DB + headless screenshot. 9 choropleth tests pass. Commit `f7d3fd39edb`. (The `teams.state`-keyed *destination*-state variant — where transfers LANDED by school-state — remains genuinely gated, but it's a nice-to-have, not the headline.) Carded surfaces now total **6** (Sankey, network, era trajectory, footprint choropleth, dynasty heatmap, transfer-origin geography).
- **In flight:** None
- **Blocked:** None — WS-08 chart-vocabulary expansion (6→9) + card shell + annotation DSL + both buildable choropleth surfaces are ALL shipped. The only un-built variant (destination-state map) is genuinely gated on empty `teams.state` and is a low-value nice-to-have.
- **Next action:** None. WS-08 is complete.
...
- **Resolvers added (session 9):** `_resolve_game_pick` (entity=game_id, predicted=winner slug; grades 1.0/0.0 vs `games` final score, 0.5 on tie, stays unresolved until the game is final) and `_resolve_award_winner` (entity=award key, predicted=winner player_id, period=season; heisman→`heisman_vote_results.winner_flag`, else `player_honors` placement=1 by honor name; stays unresolved until a winner is recorded). Both registered in `OUTCOME_RESOLVERS`. **Resolver-only by design** — no record surface was added, because game-pick/award producers are in-season and backfilling predictions after outcomes are known would corrupt calibration honesty. The grading half is ready to fire the moment in-season producers call `record_prediction`. 18 tests pass (6 new).
- **In flight:** None
- **Blocked:** Heisman surface is genuinely gated (no 2026 rankings exist yet; `heisman_vote_results` is empty everywhere so no resolvable outcome). Remaining cross-surface instrumentation (Reality Gap) + per-team track-record sections are Phase 2-3 (in-season) — the running gate is "≥10k rows in first in-season month."
- **Next action:** When the season opens: (1) wrap remaining prediction-rendering chips through `record_prediction` (game picks + award projections now have resolvers waiting); (2) add per-team "we said X, then Y" track-record section to team pages; (3) wire `resolve` into a Sunday cron (D-015 cadence B).
...
- **Shipped (2026-05-28, session 11):** Page-weight reporter (spec item 6) — `scripts/check_page_weight.py` walks `output/site`, measures **gzipped** HTML per page (the number Vercel actually serves; raw bytes overstate our deliberately inlined CSS), buckets by archetype, prints distribution (p50/p90/p99/max) + worst offenders. **Report-only by default (exit 0)** so we set honest budgets from the real distribution before gating; `--enforce --budget-kb N --max-violations M` is the future CI gate. 6 unit tests against a synthetic site. Not yet wired into CI by design (need the measured distribution first — raw team pages are ~290KB but gzip far smaller).
- **Enforced (2026-05-28, session 12):** `check_page_weight.py --enforce --budget-kb 150 --max-violations 0` is now a hard build gate in `publish_site.yml` (commit `682273003cc`), measured against the real gzip distribution; the 3 over-budget pages were externalized to JSON sidecars first (see WS-11 page-weight memory). A new inlined-payload regression fails the build. The companion color-blind palette guard runs in the same job.
- **In flight:** None
- **Blocked:** Not blocked (can audit current state anytime)
- **Next action:** Phase 5 (March 2027). Until then: enforce mobile-rendering checks in PR review (baseline = clean per the session-9 audit above).
