...
- **In flight:** Build-pipeline wiring (render era pages for all qualifying FBS programs in `build-site` + crosslink from team/program pages) — prototype proven, rollout is the next commit.
...
- **Next action:** Wire `render_all_era_pages` into the build + add the crosslink, then roll out to all FBS programs with ≥8 CFP seasons.
