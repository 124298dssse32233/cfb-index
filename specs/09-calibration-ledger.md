...
- **§7 rendered page shipped (2026-05-28):** `src/cfb_rankings/provenance/calibration_page.py` → `/methodology/calibration.html`, rendered by `build-methodology`, linked from `/methodology/`. Renders `calibration_summary` (overall + per-surface + per-band + pending state). Each pending surface also renders an **"On the record now"** receipt — a sample of the standing calls (e.g. "Alabama — 10 wins · high"), each linking to the team page, plus the resolution month ("These resolve in January 2027"). This is the differentiated "we said X" surface visible *before* outcomes exist, so the trust page is substantive in the offseason rather than an empty "awaiting outcome" blob. `publish_site.yml` populates + resolves the ledger before `build-methodology` so prod renders live data; rolling `cfb-rankings-db` artifact re-upload preserves first-seen history.
